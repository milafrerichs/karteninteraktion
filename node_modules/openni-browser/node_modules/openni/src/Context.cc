...
namespace nodeopenni {
  ...
  Context::Poll()
  {
    ...
    this->session_manager_->Update(&this->context_);
    if (hasError(status)) {
  ...
  Context::Init()
  {
    ...
    this->session_manager_ = new XnVSessionManager();
    // Start the joint pos polling thread
    this->InitPollThread();

    printf("initiated poll thread.\n");

    status = session_manager_->Initialize(&this->context_, "Wave,Click", "RaiseHand");
    if (hasError(status)) return error("init session manager", status);


    this->swipe_detector_ = new XnVSwipeDetector(false);
    //this->swipe_detector_->RegisterSwipeRight(this->context_);
    this->swipe_detector_->RegisterSwipeLeft(&this->context_,Swipe_Left);
    //this->swipe_detector_->RegisterSwipeUp(this->context_);
    //his->swipe_detector_->RegisterSwipeDown(this->context_);
    this->session_manager_->AddListener(this->swipe_detector_);
    ...
    //status = this->gesture_generator_.AddGesture("RaiseHand",this->boundingBox_);
    //status = this->gesture_generator_.AddGesture("Circle",this->boundingBox_);
    ...
    */
    /* gestGen.AddGesture("Wave");
    gestGen.AddGesture("Swipe");
    gestGen.AddGesture("RaiseHand");
    gestGen.AddGesture("Click");
    */
